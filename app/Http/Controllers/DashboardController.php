...
class DashboardController extends Controller
{
    ...
    public function _getMonthStats($month, $anio) {
        $sql = "SELECT e.nombres AS 'item', COUNT(c.id) AS 'cantidad' FROM citas c INNER JOIN empleados e ON c.empleado = e.id WHERE MONTH(c.fecha) = ? AND YEAR(c.fecha) = ? GROUP BY c.empleado
                UNION ALL
                SELECT 'TIENDA' AS 'item', COUNT(v.id) FROM ventas v WHERE MONTH(v.fecha) = ? AND YEAR(v.fecha) = ?";
